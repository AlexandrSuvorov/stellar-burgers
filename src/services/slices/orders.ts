...
>('orders/createOrder', async (ingredients, { rejectWithValue, dispatch }) => {
  try {
    ...
    return rejectWithValue('Произошла ошибка при созданий заказа');
  }
});
...
>('order/getOrders', async (_, { rejectWithValue }) => {
  try {
    ...
    if (err instanceof Error) return rejectWithValue(err.message);
    return rejectWithValue('Произошла ошибка при созданий заказов');
  }
});
...
>('orders/getOrderById', async (id, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(
      'Произошла ошибка при получений заказа по идентификатору'
    );
  }
});
...
export const getOrderModal = (state: RootState) => state.orders.orderModalData;
